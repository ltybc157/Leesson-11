...
let form = document.querySelector('.main-form'), //вызываем форму обратной связи 
    ...
    request.setRequestHeader('Content-Type', 'application/x-form-urlencoded'); //запрос к серверу 1 задаём метод обращения 2 аргументом задаём куда обращатся к форме 
   
    let formData = new FormData(form); // поместим все что написал пользователь
    request.send(formData); // он запускает запрос и обращается к серверу
